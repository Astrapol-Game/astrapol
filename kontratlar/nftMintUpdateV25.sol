...
contract GameNftV25 {
    ...
    function mint(
        ...
        // İmzayı doğrula
        bytes32 message = keccak256(abi.encode(to, tokenId, bytes(uri), level));
        emit DebugByte(message); // `message` değişkenini kontrol et

        bytes32 prefixedHash = keccak256(abi.encode("\x19Ethereum Signed Message:\n32", message));
        emit DebugByte(prefixedHash); // `prefixedHash` değişkenini kontrol et

        address signer = recoverSigner(prefixedHash, signature);
        emit DebugByte(bytes32(uint256(uint160(signer)))); // `signer` adresini bytes32 olarak gönder
