<?php
    ...
    try 
    {
        ...
        if (empty($userWallet) || empty($chainId) || empty($userNonceNewPurchase) || empty($txHash))
        {
        ...
        // yeni bir dak üretme isteğinde hazırlıklar tamamlandı, kullanıcıdan onay alındı, tüm bu işlemler için önce zincir kodu, cüzdan adresi, rastgele bir nonce
        ...
        if($sifresizChainId != $chainId)
        {
            ...
            . "Dosya = " . __FILE__ . PHP_EOL
            . "Satır = " . __LINE__ . PHP_EOL
            . "Hata Mesajı = DAK bulunamadı, yeni birtane oluşturulma işlemi başlatıldı, yeni dak üretimi sırasında yeniDakMintHazirlaBaslat.php ile yeniDakMintTxDogrula.php işlemleri arasında chain id aynı değil" . PHP_EOL
            . "userWallet = " . $userWallet . PHP_EOL
            . "chainId = " . $chainId . PHP_EOL
        ...
        if($sifresizUserWallet != $userWalletTest)
        {
            ...
            . "Dosya = " . __FILE__ . PHP_EOL
            . "Satır = " . __LINE__ . PHP_EOL
            . "Hata Mesajı = DAK bulunamadı, yeni birtane oluşturulma işlemi başlatıldı, yeni dak üretimi sırasında yeniDakMintHazirlaBaslat.php ile yeniDakMintTxDogrula.php işlemleri arasında cüzdan adresi aynı değil" . PHP_EOL
            . "userWallet = " . $userWallet . PHP_EOL
            . "chainId = " . $chainId . PHP_EOL
            . "zincir adı = " . $ayarlar['tokenZincirDetaylari'][$chainId]['adi'] . PHP_EOL
            . "userNonceNewPurchase = ". $userNonceNewPurchase . PHP_EOL
        ...
        foreach ($ayarlar['rpcOkumaAdresleri'][$chainId] as $siradakiRpcLinki) 
        {
            ...
            for ($kacDefaBakildi=1; $kacDefaBakildi <= $kacDefaDenenecek; $kacDefaBakildi++) 
            {
                ...
                if ($transaction && isset($transaction['result']))
                {
                    ...
                    $txGonderenAdres = checkSumFormatinaDonustur($transaction['result']['from']);
                    $dtPurchase = '';
                    ...
                    // DAK üretilsin mi diye soruldu, kullanıcı evet dedi, yeniDakMintHazirlaBaslat.php çalıştı, mint blockchaine gönderildi, tx hash alındı
                    // tekrar buraya gelirken kullanıcıdan zaten gerekli bilgileri POST ile aldık, şifreli veriyi çözdük ve karşılaştırdık
                    ...
                    if($txHashDataBilgisi !== $dbNonceHex)
                    {
                        ...
                        . "txHash = " . $txHash . PHP_EOL
                        . "data (nonce hex) = ". $sifresizNonceNewPurchase . PHP_EOL
                        . "txHash içindeki miktar = " . $txHashAmountFormatted . PHP_EOL
                        . "veri tabanındaki miktar = " . $dbAmountFormatted . PHP_EOL
                        . "txhash kontrolünde okunan transfer adresi = " . $txGonderenAdres . PHP_EOL
                    ...
                    if($txGonderenAdres === $dbToAddress)
                    {
                    ...
                    else
                    {
                        ...
                        . "txhash kontrolünde okunan transfer adresi = " . $txGonderenAdres . PHP_EOL
                        . "ayarlarda okunan hedef adres = " . $dbToAddress . PHP_EOL
                        . "txHashDataBilgisi = " . $txHashDataBilgisi . PHP_EOL
                        . PHP_EOL
                        , FILE_APPEND);
